...
beforeAll(async () => { env = await initializeTestEnvironment({ projectId: "phase5-rules", firestore: { rules: fs.readFileSync(path.resolve(__dirname, "../firestore.rules"), "utf8"), host: "localhost", port: Number(process.env.FIRESTORE_EMULATOR_PORT ?? 8091) } }); });
