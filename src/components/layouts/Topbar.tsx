...
// ponytail: chưa có read-tracking cho thông báo → stub rỗng (ẩn badge). Wire vào announcements khi có model "đã đọc".
type Notification = { id: string; title: string; time: string };
function useNotifications(): Notification[] { return []; }
...
function NotificationBell({ seeAllHref }: { seeAllHref: string }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="glass-panel absolute right-0 mt-2 w-80 max-w-[calc(100vw-2rem)] overflow-hidden rounded-2xl border border-white/80 shadow-[var(--shadow-3)]">
          <header className="flex items-center justify-between border-b border-neutral-200/70 px-4 py-3"><b className="text-sm">Thông báo</b></header>
          ...
          <footer className="border-t border-neutral-200/70 p-2 text-center"><Link to={seeAllHref} onClick={() => setOpen(false)} className="block rounded-xl px-3 py-2 text-sm font-semibold text-primary-600 hover:bg-white/70">Xem tất cả thông báo</Link></footer>
        </div>
      )}
    </div>
  );
