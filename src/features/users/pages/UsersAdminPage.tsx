...
function Avatar({ name, compact = false }: { name: string; compact?: boolean }) {
  return <span className={`${compact ? "size-8 text-[10px]" : "size-10 text-xs"} flex shrink-0 items-center justify-center rounded-full bg-primary-50 font-bold text-primary-700 ring-1 ring-primary-100`}>{initials(name)}</span>;
}
...
function Metric({ icon: Icon, value, label, detail, tone }: { icon: typeof UsersRound; value: string; label: string; detail: string; tone: "primary" | "success" | "warning" | "info" }) {
  ...
  return <div className="rounded-card border border-neutral-200 bg-white p-4 shadow-[var(--shadow-1)]"><div className="flex items-start justify-between gap-3"><span className={`flex size-9 items-center justify-center rounded-input ring-1 ${tones[tone]}`}><Icon size={18} /></span><span className="rounded-full bg-neutral-50 px-2.5 py-1 text-[11px] font-semibold text-neutral-500">Realtime</span></div><p className="mt-4 text-2xl font-bold tabular-nums text-neutral-900">{value}</p><p className="mt-0.5 text-sm font-semibold text-neutral-800">{label}</p><p className="mt-1 text-xs text-neutral-500">{detail}</p></div>;
}

function TabBar({ value, onChange }: { value: MainTab; onChange: (tab: MainTab) => void }) {
  return <div className="mb-4 flex gap-1 overflow-x-auto border-b border-neutral-200" role="tablist" aria-label="Điều hướng module Người dùng">{MAIN_TABS.map((tab) => <button key={tab.value} type="button" role="tab" aria-selected={value === tab.value} onClick={() => onChange(tab.value)} className={`relative min-h-touch shrink-0 px-4 text-sm font-semibold transition ${value === tab.value ? "text-primary-700" : "text-neutral-500 hover:text-neutral-800"}`}>{tab.label}{value === tab.value && <span className="absolute inset-x-3 bottom-0 h-0.5 rounded-full bg-primary-500" />}</button>)}</div>;
}
