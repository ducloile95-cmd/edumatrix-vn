...
const DEMO_ITEMS: DemoBillingGroup[] = [
  {
    id: "writing-foundation",
    name: "Bộ giáo trình IELTS Foundation",
    subtitle: "IELTS Foundation / Tiếng Anh",
    amount: 480_000,
    pricing: "flat",
    students: [
      { id: "hs001", name: "Nguyễn Minh Anh", code: "HS001" },
  ...
  {
    id: "toeic-listening",
    name: "Sách bài tập TOEIC Listening",
    subtitle: "TOEIC Essentials / Tiếng Anh",
    amount: 220_000,
    pricing: "flat",
    students: [
      { id: "hs027", name: "Lê Minh Khang", code: "HS027" },
...
export function InvoiceClassBillingDemo({ onClose }: { onClose: () => void }) {
  ...
  const billingGroups = billingMode === "class" ? DEMO_CLASSES : DEMO_ITEMS;
  ...
  const modeLabel = billingMode === "class" ? "lớp" : "đồ dùng";
  const selectionLabel = billingMode === "class" ? "lớp học" : "đồ dùng học tập";

  function changeBillingMode(mode: BillingMode) {
    setBillingMode(mode);
    setSelectedGroupId("");
    setSelectedStudentIds([]);
    setTitle(mode === "class" ? "Học phí tháng 08/2026" : "Đồ dùng học tập");
  }
      ...
      <div className="min-h-0 flex-1 overflow-y-auto bg-neutral-50/70 p-4 sm:p-6">
        <div className="mx-auto mb-4 flex max-w-[1600px] flex-col gap-3 rounded-card border border-neutral-200 bg-white p-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div role="tablist" aria-label="Chọn loại hóa đơn" className="grid grid-cols-2 gap-1 rounded-input border border-neutral-200 bg-neutral-50 p-1">
            ...
            <button type="button" role="tab" aria-selected={billingMode === "item"} onClick={() => changeBillingMode("item")} className={`min-h-10 rounded-lg px-4 text-xs font-bold transition ${billingMode === "item" ? "bg-white text-primary-700 shadow-sm" : "text-neutral-600 hover:text-neutral-900"}`}>Đồ dùng học tập</button>
          </div>
        </div>
        <div className="mx-auto grid max-w-[1600px] gap-4 xl:grid-cols-[minmax(300px,.82fr)_minmax(360px,1fr)_minmax(300px,.78fr)] xl:items-start">
          <section className={PANEL_CLASS} aria-labelledby="invoice-demo-class-heading">
            <PanelHeader
              icon={billingMode === "class" ? <School size={17} aria-hidden="true" /> : <PackageCheck size={17} aria-hidden="true" />}
              id="invoice-demo-class-heading"
              title={billingMode === "class" ? "Chọn lớp lập học phí" : "Chọn đồ dùng học tập"}
              description={billingMode === "class" ? "Mở từ Lớp học, cột Thao tác." : "Đơn giá lấy từ danh mục đồ dùng."}
            />
            <div className="overflow-x-auto">
              <table className="w-full min-w-[300px] text-left text-sm">
                <thead className="border-b border-neutral-200 bg-neutral-50 text-xs font-bold text-neutral-500">
                  <tr>
                <th className="px-4 py-2.5" scope="col">{billingMode === "class" ? "Lớp học" : "Đồ dùng"}</th>
                ...
                <tbody className="divide-y divide-neutral-100">
                  {billingGroups.map((item) => {
                    ...
                    <p className="mt-0.5 text-xs text-neutral-500">{item.students.length} học sinh / {formatVnd(item.amount)}{item.pricing === "session" ? "/buổi" : ""}</p>
                        </td>
          ...
          <section className={PANEL_CLASS} aria-labelledby="invoice-demo-detail-heading">
            ...
            <div className="grid gap-4 p-4">
              ...
              <div className="flex items-center justify-between gap-3 text-xs text-neutral-600"><span>{billingMode === "class" ? "Học phí mỗi học sinh" : "Đồ dùng / học sinh"}</span><strong className="tabular-nums text-neutral-900">{formatVnd(amountPerStudent)}</strong></div>
