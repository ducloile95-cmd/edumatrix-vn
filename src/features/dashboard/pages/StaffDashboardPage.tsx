...
// Lazy: recharts (~450KB) chi tai khi thuc su ve bieu do, khong chan first paint dashboard.
const QualityCharts = lazy(() => import("@/features/dashboard/components/QualityCharts"));
const FinanceAgingChart = lazy(() => import("@/features/dashboard/components/FinanceAgingChart"));
    ...
    <div className="space-y-4">
      ...
      {tab === "quality" && <QueryPanel loading={learning.isLoading} error={learning.isError || academic.isError} retry={() => { academic.refetch(); learning.refetch(); }}>
        {learning.data && <>
          ...
          <Suspense fallback={<div className="grid gap-4 lg:grid-cols-2"><LoadingSkeleton rows={5} /><LoadingSkeleton rows={5} /></div>}>
            <QualityCharts days={days} attendanceTrend={learning.data.attendanceTrend} rankDistribution={learning.data.rankDistribution} reducedMotion={reducedMotion} />
          </Suspense>
