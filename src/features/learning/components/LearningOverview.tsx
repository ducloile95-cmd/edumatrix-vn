    ...
    <div className="space-y-4">
      ...
      <section className="grid gap-4 xl:grid-cols-[1.45fr_.85fr]">
        <ChartPanel title="Xu hướng kết quả" description="Điểm trung bình theo các bài đánh giá gần nhất" className="min-h-[340px]">
          {analytics.scoreTrend.length ? <ResponsiveContainer width="100%" height="100%"><AreaChart data={analytics.scoreTrend} margin={{ top: 14, right: 8, left: -20, bottom: 0 }}>{ChartGradientDefs()}<CartesianGrid stroke={CHART_GRID_COLOR} strokeDasharray="3 5" vertical={false}/><XAxis dataKey="name" tick={CHART_AXIS_TICK} axisLine={false} tickLine={false}/><YAxis domain={[0,100]} tick={CHART_AXIS_TICK} axisLine={false} tickLine={false}/><Tooltip contentStyle={CHART_TOOLTIP_STYLE} formatter={(value:number)=>[`${value}%`,"Trung bình"]}/><Area type="monotone" dataKey="average" stroke={CHART_GRADIENT.primary} strokeWidth={4} fill={CHART_GRADIENT.area} filter={CHART_GLOW_FILTER} activeDot={{ r: 6, fill: "#fff", stroke: CHART_PRIMARY, strokeWidth: 3 }} isAnimationActive={!reducedMotion} animationDuration={CHART_ANIMATION_DURATION}/></AreaChart></ResponsiveContainer> : <ChartEmpty text="Chưa có dữ liệu điểm." />}
        </ChartPanel>

        <ChartPanel title="Luồng bài tập" description="Từ giao bài đến hoàn tất chấm" className="min-h-[340px]">
          <div className="grid h-full grid-rows-[minmax(0,1fr)_auto] gap-2"><div className="min-h-0"><ResponsiveContainer width="100%" height="100%"><PieChart>{ChartGradientDefs()}<Pie data={funnel} dataKey="value" nameKey="label" innerRadius="56%" outerRadius="84%" paddingAngle={4} cornerRadius={8} filter={CHART_DEPTH_FILTER} isAnimationActive={!reducedMotion} animationDuration={CHART_ANIMATION_DURATION}>{funnel.map((item)=><Cell key={item.key} fill={item.key === "assigned" ? CHART_GRADIENT.primarySoft : item.key === "submitted" ? CHART_GRADIENT.info : item.key === "graded" ? CHART_GRADIENT.success : CHART_GRADIENT.danger}/>)}</Pie><Tooltip contentStyle={CHART_TOOLTIP_STYLE} formatter={(value:number)=>[`${value} lượt`,"Số lượng"]}/><text x="50%" y="47%" textAnchor="middle" fill="#1C1A15" fontSize="26" fontWeight="700">{totals.assigned}</text><text x="50%" y="57%" textAnchor="middle" fill={CHART_AXIS_TICK.fill} fontSize="11">lượt được giao</text></PieChart></ResponsiveContainer></div>
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <ChartPanel title="Tỉ lệ nộp bài theo lớp" description="So sánh mức độ hoàn thành giữa các lớp" className="min-h-[330px]">
          {analytics.submissionByClass.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={analytics.submissionByClass} layout="vertical" margin={{ top: 4, right: 20, left: 4, bottom: 0 }}>{ChartGradientDefs()}<CartesianGrid stroke={CHART_GRID_COLOR} strokeDasharray="3 5" horizontal={false}/><XAxis type="number" domain={[0,100]} hide/><YAxis type="category" dataKey="name" width={100} tick={CHART_AXIS_TICK} axisLine={false} tickLine={false}/><Tooltip contentStyle={CHART_TOOLTIP_STYLE} formatter={(value:number)=>[`${value}%`,"Đã nộp"]}/><Bar dataKey="percent" fill={CHART_GRADIENT.primary} filter={CHART_DEPTH_FILTER} radius={[0,10,10,0]} barSize={18} isAnimationActive={!reducedMotion} animationDuration={CHART_ANIMATION_DURATION}/></BarChart></ResponsiveContainer> : <ChartEmpty text="Chưa có dữ liệu bài nộp." />}
        </ChartPanel>

        <ChartPanel title="Phân phối điểm" description="Số lượt đánh giá theo từng khoảng điểm" className="min-h-[330px]">
          <ResponsiveContainer width="100%" height="100%"><BarChart data={analytics.distribution} margin={{ top: 12, right: 6, left: -20, bottom: 0 }}>{ChartGradientDefs()}<CartesianGrid stroke={CHART_GRID_COLOR} strokeDasharray="3 5" vertical={false}/><XAxis dataKey="label" tick={CHART_AXIS_TICK} axisLine={false} tickLine={false}/><YAxis allowDecimals={false} tick={CHART_AXIS_TICK} axisLine={false} tickLine={false}/><Tooltip contentStyle={CHART_TOOLTIP_STYLE} formatter={(value:number)=>[`${value} lượt`,"Số lượng"]}/><Bar dataKey="count" fill={CHART_GRADIENT.primarySoft} filter={CHART_DEPTH_FILTER} radius={[10,10,3,3]} barSize={32} isAnimationActive={!reducedMotion} animationDuration={CHART_ANIMATION_DURATION}/></BarChart></ResponsiveContainer>
        </ChartPanel>
      </section>
